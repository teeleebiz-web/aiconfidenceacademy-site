...
it('shows one approved video without a misplaced workbook tab',()=>{
  ...
  const {container}=render(<JourneyIntroductionView introduction={current} mediaUrl="/approved-video.mp4" captionUrl={null} onBack={()=>undefined} onContinue={()=>undefined} />)
  ...
  expect(screen.queryByRole('link',{name:'Open Workbook'})).toBeNull()
})
