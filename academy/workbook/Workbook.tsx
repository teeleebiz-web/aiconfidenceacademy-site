...
export function Workbook({ lessonId, workbookKey = 'journey-one' }: { lessonId?: string | null; workbookKey?: 'journey-one' | 'journey-three' | 'journey-four' | 'journey-five' }) {
  ...
  const openingPage = workbookKey === 'journey-five' ? 1 : workbookKey === 'journey-four' ? (lessonId === '4.2' ? 5 : 1) : workbookKey === 'journey-three' ? (lessonId === '3.1' ? 1 : lessonId === '3.2' ? 7 : lessonId === '3.3' ? 11 : lessonId === '3.4' ? 15 : lessonId === '3.5' ? 19 : lessonId === '3.6' ? 23 : undefined) : /^1\.[1-6]$/.test(lessonId ?? '') ? 5 + (Number(lessonId!.split('.')[1]) - 1) * 4 : undefined
  useEffect(() => {
    let active = true
