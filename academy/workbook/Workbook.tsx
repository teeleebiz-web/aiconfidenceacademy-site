...
export function Workbook({ lessonId, workbookKey = 'journey-one' }: { lessonId?: string | null; workbookKey?: 'journey-one' | 'journey-three' | 'journey-four' }) {
  ...
  const openingPage = workbookKey === 'journey-four' ? (lessonId === '4.2' ? 5 : 1) : workbookKey === 'journey-three' ? (lessonId === '3.1' ? 1 : lessonId === '3.2' ? 7 : lessonId === '3.3' ? 11 : lessonId === '3.4' ? 15 : lessonId === '3.5' ? 19 : lessonId === '3.6' ? 23 : undefined) : /^1\.[1-6]$/.test(lessonId ?? '') ? 5 + (Number(lessonId!.split('.')[1]) - 1) * 4 : undefined
  useEffect(() => {
    let active = true
  ...
  return <main className="wb-shell">
    ...
    <div className="wb-layout">
      <nav className="wb-navigation" aria-label="Workbook pages"><p className="wb-eyebrow">{data.navigationLabel ?? 'Journey One workbook'}</p><label htmlFor="wb-page-select">Choose a page</label><select id="wb-page-select" value={state.last_page} onChange={e => move(Number(e.target.value))}>{data.pages.map(p => <option key={p.number} value={p.number} disabled={!state.allowedPages.includes(p.number)}>{p.number}. {p.title}</option>)}</select><p>Your answers save as you write. You can also select Save.</p><div className="wb-chapters">{(data.contents ?? [{ page:2, text:'Your workbook guide' }, ...[1,2,3,4,5,6].map(n => ({ page:5+(n-1)*4, text:`${n}. ${data.pages.find(p => p.number === 5+(n-1)*4)?.title ?? 'Lesson available later'}` })),{ page:29, text:'Self Check Guide' },{ page:31, text:'Practice Log' }]).map(item => <button key={item.page} onClick={() => move(item.page)} disabled={!state.allowedPages.includes(item.page)} aria-current={((workbookKey === 'journey-three' || workbookKey === 'journey-four') ? state.last_page === item.page : state.last_page >= item.page && state.last_page < (item.page <= 25 && item.page >= 5 ? item.page+4 : item.page+2)) ? 'page' : undefined}>{item.text}</button>)}</div></nav>
