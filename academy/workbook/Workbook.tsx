...
const transport: Transport = async (method, patch): Promise<RecordState> => {
  ...
  const response = await fetch('/api/academy/workbooks/journey-one', {
...
export function Workbook({ lessonId }: { lessonId?: string | null }) {
  ...
  return <main className="wb-shell">
    ...
    <div className="wb-layout">
      <nav className="wb-navigation" aria-label="Workbook pages"><p className="wb-eyebrow">Journey One workbook</p><label htmlFor="wb-page-select">Choose a page</label><select id="wb-page-select" value={state.last_page} onChange={e => move(Number(e.target.value))}>{data.pages.map(p => <option key={p.number} value={p.number} disabled={!state.allowedPages.includes(p.number)}>{p.number}. {p.title}</option>)}</select><p>Your answers save as you write. You can also select Save.</p><div className="wb-chapters">{[{ page:2, text:'Your workbook guide' }, ...[1,2,3,4,5,6].map(n => ({ page:5+(n-1)*4, text:`${n}. ${data.pages.find(p => p.number === 5+(n-1)*4)?.title ?? 'Lesson available later'}` })),{ page:29, text:'Self Check Guide' },{ page:31, text:'Practice Log' }].map(item => <button key={item.page} onClick={() => move(item.page)} disabled={!state.allowedPages.includes(item.page)} aria-current={state.last_page >= item.page && state.last_page < (item.page <= 25 && item.page >= 5 ? item.page+4 : item.page+2) ? 'page' : undefined}>{item.text}</button>)}</div></nav>
      <article className="wb-paper" aria-label={`Workbook page ${page.number}`}>
        <header className="wb-page-hero"><p className="wb-eyebrow">{page.kicker.replaceAll('  /  ', ' · ')}</p><h1 ref={titleRef} tabIndex={-1}>{page.title}</h1>{page.number === 1 && <><img src={sealUrl} alt="AI Confidence Academy seal" /><p>Learner Workbook and Practice Log · Lessons 1.1–1.6</p></>}</header>
        <div className="wb-page-body">{page.blocks.map((block, i) => render(block as Block, i))}</div>
        <footer className="wb-page-footer"><span>AI Assists. Humans Verify.</span><span>Page {page.number} of 32</span></footer>
