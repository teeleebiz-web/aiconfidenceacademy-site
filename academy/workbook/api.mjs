...
const configurations = {
  'journey-four': { journey: 4, pages: [1, 2, 3, 4], start: 1, fallback: 1 },
...
export async function workbookIdentity(req, db, courseId, ownerAuthenticated, workbookKey = 'journey-one') {
  ...
  const allowedPages = config.pages.filter(page => workbookKey === 'journey-four' ? lessonNumbers.includes(1) : workbookKey === 'journey-three' ? lessonNumbers.includes(page <= 6 ? 1 : page <= 10 ? 2 : page <= 14 ? 3 : page <= 18 ? 4 : page <= 22 ? 5 : 6) : page <= 4 || page >= 29 || lessonNumbers.includes(Math.floor((page - 5) / 4) + 1))
