...
const configurations = {
  ...
  'journey-six': { journey: 6, pages: [1, 2, 3, 4], start: 1, fallback: 1 },
...
export async function workbookIdentity(req, db, courseId, ownerAuthenticated, workbookKey = 'journey-one') {
  ...
  const allowedPages = config.pages.filter(page => workbookKey === 'journey-six' ? lessonNumbers.includes(Math.floor((page - 1) / 4) + 1) : workbookKey === 'journey-five' ? lessonNumbers.includes(Math.floor((page - 1) / 4) + 1) : workbookKey === 'journey-four' ? lessonNumbers.includes(page <= 4 ? 1 : 2) : workbookKey === 'journey-three' ? lessonNumbers.includes(page <= 6 ? 1 : page <= 10 ? 2 : page <= 14 ? 3 : page <= 18 ? 4 : page <= 22 ? 5 : 6) : page <= 4 || page >= 29 || lessonNumbers.includes(Math.floor((page - 5) / 4) + 1))
