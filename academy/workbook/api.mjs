...
export async function workbookIdentity(req, db, courseId, ownerAuthenticated) {
  const token = req.headers['x-aca-access-token']
  if (!token) {
    if (!ownerAuthenticated) throw fault(401, 'Please sign in to your Academy account.')
    return { scope: 'owner-review', learnerId: null, allowedPages: allPages }
  }
  ...
  const j = await db.from('course_journeys').select('id,status').eq('course_id', courseId).eq('journey_number', 1).maybeSingle()
  ...
  const allowedPages = allPages.filter(page => page <= 4 || page >= 29 || lessonNumbers.includes(Math.floor((page - 5) / 4) + 1))
  return { scope: id, learnerId: id, allowedPages }
}

export async function handleWorkbook(req, res, { db, courseId, ownerAuthenticated }) {
  ...
  try {
    ...
    const identity = await workbookIdentity(req, db, courseId, ownerAuthenticated)
    const definition = await db.from('aca_workbook_definitions').select('content').eq('course_id', courseId).eq('workbook_key', 'journey-one').maybeSingle()
    ...
    const where = query => query.eq('course_id', courseId).eq('workbook_key', 'journey-one').eq('scope_key', identity.scope)
    ...
    const current = result.data ?? { answers: {}, last_page: identity.allowedPages.includes(5) ? 5 : 2, revision: 0, updated_at: null }
    const present = row => ({ ...row, answers: Object.fromEntries(Object.entries(row.answers).filter(([key]) => identity.allowedPages.includes(fieldPages.get(key)))), last_page: identity.allowedPages.includes(row.last_page) ? row.last_page : 2, allowedPages: identity.allowedPages, scope: identity.scope, workbook: { ...workbook, pages: workbook.pages.filter(page => identity.allowedPages.includes(page.number)) } })
    ...
    const saved = current.revision === 0
      ? await db.from('aca_workbook_responses').insert({ ...next, course_id: courseId, workbook_key: 'journey-one', scope_key: identity.scope, learner_id: identity.learnerId }).select(columns).single()
