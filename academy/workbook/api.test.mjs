  ...
  const db={ records,writes,auth:{async getUser(token){return token==='bad'?{error:Error('invalid')}:{data:{user:{id:token,is_anonymous:false}}}}},from(table){
    ...
    const query={
      ...
      async run(single){
        ...
        else if(table==='course_journeys')rows=[{id:'journey',course_id:'course',journey_number:1,status:'published'}]
        else if(table==='lessons')rows=Array.from({length:6},(_,i)=>({journey_id:'journey',course_id:'course',page_id:`1.${i+1}`,status:'published',unlock_offset_days:i}))
        else if(table==='aca_workbook_definitions')rows=[{course_id:'course',workbook_key:'journey-one',content:workbook}]
