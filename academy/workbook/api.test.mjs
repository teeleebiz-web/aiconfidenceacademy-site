...
const fifthWorkbook={key:'journey-five',version:5,title:'Fifth workbook fixture',pages:Array.from({length:20},(_,i)=>({number:i+1,kicker:'Practice',title:`Fifth page ${i+1}`,blocks:[{type:'field',id:`fifth-${i+1}`,label:'My answer'}]}))}
const ids=[]
...
function database(thirdLessonStatus = 'draft', lesson33Status = 'draft', lesson34Status = 'draft', lesson35Status = 'draft', lesson36Status = 'draft', lesson41Status = 'draft', lesson42Status = 'draft', lesson51Status = 'draft', lesson52Status = 'draft', lesson53Status = 'draft', lesson54Status = 'draft', lesson55Status = 'draft') {
...
test('Journey Five workbook is private, separate, and opens only with published Lesson 5.1',()=>fixture(async({request,url,headers})=>{
  ...
  assert.deepEqual(initial.allowedPages,Array.from({length:20},(_,i)=>i+1))
  ...
  assert.equal((await patch({baseRevision:1,page:21,answers:{}})).status,400)
