...
function Academy() {
  ...
  return <div className="portal-shell">
    ...
    {data && (setup ? <GettingStarted onBack={() => setSetup(false)} onContinue={data.lessons.some(l => l.page_id === '1.1') ? () => openLesson(data.lessons.find(l => l.page_id === '1.1')!) : undefined} /> : welcome ? <JourneyIntroductionView {...welcome} onBack={() => setWelcome(null)} onContinue={() => { const next = data.lessons.find(l => l.journey_id === welcome.introduction.journey_id); if(next) openLesson(next) }} />
      : lesson ? <>
        {data.journeys.some(journey => journey.journey_number === 2 && journey.id === lesson.journey_id) && <nav className="portal-main" aria-label="Journey Two lessons"><div className="lesson-list">{data.lessons.filter(item => item.journey_id === lesson.journey_id).map(item => <button type="button" className="lesson-row" key={item.id} aria-current={item.id === lesson.id ? 'page' : undefined} aria-label={`Open Lesson ${item.page_id}: ${item.title}`} onClick={() => openLesson(item)}><span>{item.page_id}</span><strong>{item.title}</strong><span>Open lesson</span></button>)}</div></nav>}
