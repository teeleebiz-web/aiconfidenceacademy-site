...
function Academy() {
  ...
  function openLesson(next: Lesson) {
    ...
    if (/^4\.[1-6]$/.test(next.page_id)) window.history.replaceState(null, '', `/academy/phase-one/?lesson=${encodeURIComponent(next.page_id)}`)
    setSetup(false); setWelcome(null); setLesson(next); window.scrollTo(0, 0)
  }
  return <div className={`portal-shell${(isWorkbook && workbookKey === 'journey-four') || (!isWorkbook && !setup && !welcome && /^4\.[1-6]$/.test(lesson?.page_id ?? '')) ? ' aca-lesson-41' : ''}`}>
    ...
    {!isWorkbook && data && (setup ? <GettingStarted onBack={() => setSetup(false)} onContinue={data.lessons.some(l => l.page_id === '1.1') ? () => openLesson(data.lessons.find(l => l.page_id === '1.1')!) : undefined} /> : welcome ? <JourneyIntroductionView {...welcome} workbookHref={welcome.introduction.content.roadmap[0]?.page_id === '3.1' ? '/academy/phase-one/?workbook=journey-three&lesson=3.1' : undefined} onBack={() => setWelcome(null)} onContinue={() => { const next = data.lessons.find(l => l.journey_id === welcome.introduction.journey_id); if(next) openLesson(next) }} />
      : lesson ? <>
        <LessonView key={lesson.id} lesson={lesson} workbookHref={/^4\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-four&lesson=${encodeURIComponent(lesson.page_id)}` : /^1\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-one&lesson=${encodeURIComponent(lesson.page_id)}` : ['3.1', '3.2', '3.3', '3.4', '3.5', '3.6'].includes(lesson.page_id) ? `/academy/phase-one/?workbook=journey-three&lesson=${encodeURIComponent(lesson.page_id)}` : undefined} videoSrc={lesson.content.video_path ? `/api/academy/lesson-video/${encodeURIComponent(lesson.id)}?v=${encodeURIComponent(lesson.content.video_path)}` : undefined} audioSrc={lesson.content.audio_path ? `/api/academy/lesson-audio/${encodeURIComponent(lesson.id)}?v=${encodeURIComponent(lesson.content.audio_path)}` : undefined} audioPreload={data.journeys.some(journey => journey.journey_number === 2 && journey.id === lesson.journey_id) ? 'metadata' : 'none'} reviewMode previousLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)-1]} nextLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)+1]} onBack={() => setLesson(null)} onOpenLesson={openLesson} onSave={async () => { throw new Error('Progress recording is unavailable during construction review.') }} />
      </>
