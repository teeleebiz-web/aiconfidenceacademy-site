...
function Academy() {
  ...
  function openLesson(next: Lesson) { setWelcome(null); setLesson(next); window.scrollTo(0, 0) }
  return <div className="portal-shell">
    ...
    {data && (welcome ? <JourneyIntroductionView {...welcome} onBack={() => setWelcome(null)} onContinue={() => { const next = data.lessons.find(l => l.journey_id === welcome.introduction.journey_id); if(next) openLesson(next) }} />
      : lesson ? <LessonView key={lesson.id} lesson={lesson} reviewMode previousLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)-1]} nextLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)+1]} onBack={() => setLesson(null)} onOpenLesson={openLesson} onSave={async () => { throw new Error('Progress recording is unavailable during construction review.') }} />
      : <main className="portal-main"><section className="welcome-panel"><div><p className="eyebrow">Phase One</p><h1>{data.course.title}</h1><p>{data.course.summary}</p></div></section><section className="course-panel">{data.journeys.map(journey => {
        const intro = data.introductions.find(i => i.journey_id === journey.id)
        return <article className="journey-card" key={journey.id}><div className="journey-number">Journey {journey.journey_number}</div><div className="journey-copy"><h2>{journey.title}</h2><p>{journey.promise}</p>{intro && <button className="journey-welcome-button" disabled={busy} onClick={() => openWelcome(intro.id)}>{intro.content.title}</button>}<div className="lesson-list">{data.lessons.filter(l => l.journey_id === journey.id).map(l => <button className="lesson-row" key={l.id} onClick={() => openLesson(l)}><span>{l.page_id}</span><strong>{l.title}</strong><span>Open lesson</span></button>)}</div></div></article>
      })}</section></main>)}
  </div>
}
