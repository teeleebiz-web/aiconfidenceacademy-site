...
function Academy() {
  ...
  const workbookKey = requestedWorkbook === 'journey-six' ? 'journey-six' : requestedWorkbook === 'journey-five' ? 'journey-five' : requestedWorkbook === 'journey-four' ? 'journey-four' : requestedWorkbook === 'journey-three' ? 'journey-three' : requestedWorkbook === 'journey-two' ? 'journey-two' : 'journey-one'
  const isWorkbook = requestedWorkbook === 'journey-one' || requestedWorkbook === 'journey-two' || requestedWorkbook === 'journey-three' || requestedWorkbook === 'journey-four' || requestedWorkbook === 'journey-five' || requestedWorkbook === 'journey-six'
  ...
  return <AcademyShell activeSection={activeSection} workbookHref={activeWorkbookHref}>
    ...
    {!isWorkbook && data && (academyWelcome ? <main className="academy-welcome-main"><section className="academy-welcome-panel" aria-labelledby="academy-welcome-title"><p className="eyebrow">AI Confidence Academy</p><h1 id="academy-welcome-title">Welcome to the Academy</h1><p className="academy-welcome-introduction">Before you begin Journey 1, hear from the founders and instructors.</p><div className="academy-welcome-player"><video controls preload="metadata" playsInline src="/api/academy/academy-welcome-video">Your browser does not support the Academy welcome film.</video></div><button className="academy-welcome-continue" onClick={() => { window.history.replaceState(null, '', '/academy/phase-one/?view=getting-started'); setAcademyWelcome(false); setSetup(true); window.scrollTo(0, 0) }}>Continue to ChatGPT</button></section></main> : setup ? <GettingStarted onBack={() => setSetup(false)} onContinue={data.lessons.some(l => l.page_id === '1.1') ? () => openLesson(data.lessons.find(l => l.page_id === '1.1')!) : undefined} /> : welcome ? <JourneyIntroductionView {...welcome} onBack={() => setWelcome(null)} onContinue={() => { const next = data.lessons.find(l => l.journey_id === welcome.introduction.journey_id); if(next) openLesson(next) }} />
      : lesson ? <>
        <LessonView key={lesson.id} lesson={lesson} workbookHref={/^6\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-six&lesson=${encodeURIComponent(lesson.page_id)}` : /^5\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-five&lesson=${encodeURIComponent(lesson.page_id)}` : /^4\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-four&lesson=${encodeURIComponent(lesson.page_id)}` : /^3\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-three&lesson=${encodeURIComponent(lesson.page_id)}` : /^2\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-two&lesson=${encodeURIComponent(lesson.page_id)}` : /^1\.[1-6]$/.test(lesson.page_id) ? `/academy/phase-one/?workbook=journey-one&lesson=${encodeURIComponent(lesson.page_id)}` : undefined} videoSrc={lesson.content.video_path ? `/api/academy/lesson-video/${encodeURIComponent(lesson.id)}?v=${encodeURIComponent(lesson.content.video_path)}` : undefined} audioSrc={lesson.content.audio_path ? `/api/academy/lesson-audio/${encodeURIComponent(lesson.id)}?v=${encodeURIComponent(lesson.content.audio_path)}` : undefined} audioPreload={data.journeys.some(journey => journey.journey_number === 2 && journey.id === lesson.journey_id) ? 'metadata' : 'none'} reviewMode previousLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)-1]} nextLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)+1]} onBack={() => setLesson(null)} onOpenLesson={openLesson} onSave={async () => { throw new Error('Progress recording is unavailable during construction review.') }} />
      </>
