...
function Academy() {
  ...
  return <div className="portal-shell">
    ...
    {data && (setup ? <GettingStarted onBack={() => setSetup(false)} onContinue={data.lessons.some(l => l.page_id === '1.1') ? () => openLesson(data.lessons.find(l => l.page_id === '1.1')!) : undefined} /> : welcome ? <JourneyIntroductionView {...welcome} onBack={() => setWelcome(null)} onContinue={() => { const next = data.lessons.find(l => l.journey_id === welcome.introduction.journey_id); if(next) openLesson(next) }} />
      : lesson ? <>
        {data.journeys.some(journey => journey.journey_number === 2 && journey.id === lesson.journey_id) && <nav className="portal-main" aria-label="Journey Two lessons" style={{ paddingBottom: 0 }}><div className="lesson-list" style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{data.lessons.filter(item => item.journey_id === lesson.journey_id).map(item => <button type="button" className="lesson-row" style={{ display: 'block', width: 'auto', padding: '.65rem 1rem' }} key={item.id} aria-current={item.id === lesson.id ? 'page' : undefined} aria-label={`Open Lesson ${item.page_id}: ${item.title}`} onClick={() => openLesson(item)}>{item.page_id}</button>)}</div></nav>}
        <LessonView key={lesson.id} lesson={lesson} videoSrc={lesson.content.video_path ? `/api/academy/lesson-video/${encodeURIComponent(lesson.id)}?v=${encodeURIComponent(lesson.content.video_path)}` : undefined} audioSrc={lesson.content.audio_path ? `/api/academy/lesson-audio/${encodeURIComponent(lesson.id)}?v=${encodeURIComponent(lesson.content.audio_path)}` : undefined} audioPreload={data.journeys.some(journey => journey.journey_number === 2 && journey.id === lesson.journey_id) ? 'metadata' : 'none'} reviewMode previousLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)-1]} nextLesson={data.lessons[data.lessons.findIndex(l => l.id === lesson.id)+1]} onBack={() => setLesson(null)} onOpenLesson={openLesson} onSave={async () => { throw new Error('Progress recording is unavailable during construction review.') }} />
      </>
